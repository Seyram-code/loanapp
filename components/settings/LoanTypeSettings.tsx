...
type LoanType = { id: string; name: string; description: string | null; defaultInterestRate: string; defaultTerm: number; repaymentFrequency: string; minimumAmount: string; maximumAmount: string; status: 'ACTIVE' | 'INACTIVE'; _count?: { loans: number } }
type FormState = { name: string; description: string; defaultInterestRate: string; defaultTerm: string; repaymentFrequency: string; minimumAmount: string; maximumAmount: string }
const emptyForm: FormState = { name: '', description: '', defaultInterestRate: '', defaultTerm: '', repaymentFrequency: 'MONTHLY', minimumAmount: '', maximumAmount: '' }
...
export default function LoanTypeSettings() {
  ...
  function editLoanType(loanType: LoanType) { setEditingId(loanType.id); setForm({ name: loanType.name, description: loanType.description ?? '', defaultInterestRate: String(loanType.defaultInterestRate), defaultTerm: String(loanType.defaultTerm), repaymentFrequency: loanType.repaymentFrequency, minimumAmount: String(loanType.minimumAmount), maximumAmount: String(loanType.maximumAmount) }); window.scrollTo({ top: 0, behavior: 'smooth' }) }
